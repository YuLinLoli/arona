...
object DataBaseProvider {
  ...
  private fun databaseFile(dataBaseName: String): File =
    RuntimeServices.dataRoot?.resolve(dataBaseName)?.toFile() ?: Arona.dataFolderFile("/$dataBaseName")

  private fun connectDataBase(dataBaseName: String) {
    var tmp = Connection(null, ConnectionStatus.DISCONNECTED)
    runCatching {
      tmp = Connection(
        Database.connect("jdbc:sqlite:${databaseFile(dataBaseName).absolutePath}", "org.sqlite.JDBC")
      )
    }.onFailure {
      databaseConnectionList.add(tmp) //失败也插入，否则ID就串行了
      databaseConnectionList.last().connectionStatus = ConnectionStatus.DISCONNECTED
      RuntimeLog.error("Database ($dataBaseName) initialization failed. Any operation that requires database support will not be performed.")
    }.onSuccess {
  ...
  private fun initDataBase(id: Int) {
    query(id) {
      when (id) {
        ...
        else -> RuntimeLog.warning("Undefined database id : $id.")
      }

      it.addLogger(object : SqlLogger {
        override fun log(context: StatementContext, transaction: Transaction) {
          RuntimeLog.verbose { "SQL: ${context.expandArgs(transaction)}" }
        }
      })
    }
  }
